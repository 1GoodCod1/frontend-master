...
export default function ClientChatPage() {
  ...
  const handleSelectConversation = (id: string) => {
    ...
    navigate(`/client-dashboard/chat/${id}`);
  };
  ...
  if (!isLgUp) {
    return (
      <div className="flex flex-col min-h-0" style={{ height: layoutHeight }}>
        <PageHeader title={t('clientDashboard.chat')} subtitle={t('clientDashboard.chatSubtitle')} className="shrink-0" />
      ...
      <div className="flex flex-1 min-h-0 overflow-hidden rounded-xl sm:rounded-2xl border border-slate-200/80 dark:border-white/[0.08] bg-white dark:bg-black/40 dark:backdrop-blur-xl shadow-[0_2px_10px_-3px_rgba(6,81,237,0.1)] dark:shadow-none" style={{ minHeight: 0 }}>
        <div className="flex w-[320px] xl:w-[360px] min-w-0 shrink-0 flex-col overflow-hidden">
          <div className="border-b border-slate-200/80 dark:border-white/[0.08] bg-slate-50/80 dark:bg-white/[0.04] px-3 sm:px-4 py-2.5 sm:py-3">
            <h2 className="text-sm sm:text-base font-bold tracking-tight text-foreground">{t('clientDashboard.myChats')}</h2>
          </div>
        ...
        <div className="min-w-0 flex-1 overflow-hidden flex flex-col bg-slate-50/40 dark:bg-white/[0.02]">
          {effectiveSelected ? (
            ...
            <div className="flex h-full min-h-[240px] sm:min-h-[320px] flex-col items-center justify-center gap-3 sm:gap-4 p-4 sm:p-8 text-center bg-slate-50/60 dark:bg-white/[0.03] border-2 border-dashed border-slate-200 dark:border-white/[0.08] m-2 sm:m-3 rounded-xl">
              ...
              <p className="text-xs sm:text-sm font-medium text-foreground">{t('clientDashboard.selectChat')}</p>
              <p className="max-w-xs text-[11px] sm:text-xs text-muted-foreground">{t('clientDashboard.selectChatHint', 'Choose a conversation from the list to start messaging.')}</p>
            </div>
          )}
        </div>
      </div>
