...
export default function BulkActions({
  ...
  return (
    <div className="p-4 mb-4 rounded-xl border border-primary/20 bg-primary/5 animate-in fade-in duration-200">
      <Label className="text-sm font-semibold text-foreground mb-3 block">Bulk request actions</Label>
      <div className="flex flex-wrap items-center gap-3">
        <Select value={bulkStatus} onValueChange={(v) => onBulkStatusChange(v as StatusOption)}>
          <SelectTrigger className="w-[220px] h-9" aria-label="Change request status to">
            <SelectValue placeholder="Change request status to" />
          </SelectTrigger>
          <SelectContent>
        ...
        <Button disabled={isLoading || selection.length === 0} onClick={onApply}>
          Apply to {selection.length || 0} selected requests
        </Button>
        {selection.length > 0 && (
          <Badge
            variant="secondary"
          ...
          >
            {selection.length} request(s) selected ×
          </Badge>
        )}
      </div>
    </div>
